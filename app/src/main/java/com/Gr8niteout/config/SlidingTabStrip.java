    ...
    public void setTabTitlesTextColor(int selectedPosition) {
        for (int i = 0; i < getChildCount(); i++) {
            if (TextView.class.isInstance(getChildAt(i))) {
                ((TextView) getChildAt(i)).setTextColor((selectedPosition == i) ? getTabColorizer().getIndicatorColor(i) : Color.parseColor("#FFFFFF"));
                Typeface typeface = Typeface
                        .createFromAsset(getContext().getAssets(), "fonts/" + CommonUtilities.AvenirLTStd_Medium);
                ((TextView) getChildAt(i)).setTypeface(typeface);
                ((TextView) getChildAt(i)).setTextSize(14);
            }
        }
    }
